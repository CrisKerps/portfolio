...
if (document.title.includes("Attendance Senate")) {
  propublica = "senate";
} else {
  propublica = "house";
}
        ...
        partyMembers() {
          this.members.forEach((member) => {
            if (member.party == "D") {
              ...
              this.membersParty[0].votes += member.missed_votes;
              this.totalVotes += member.missed_votes;
            }
            if (member.party == "R") {
              this.membersParty[1].total++;
              this.membersParty[1].votes += member.missed_votes;
              this.totalVotes += member.missed_votes;
            }
            if (member.party == "ID") {
              this.membersParty[2].total++;
              this.membersParty[2].votes += member.missed_votes;
              this.totalVotes += member.missed_votes;
            }
            this.totalMembers++;
          });

          this.membersParty[0].votesPct = Math.trunc(
            this.membersParty[0].votes / this.membersParty[0].total
          );
          this.membersParty[1].votesPct = Math.trunc(
            this.membersParty[1].votes / this.membersParty[1].total
          );

          if (this.membersParty[2].total != 0) {
            this.membersParty[2].votesPct = Math.trunc(
              this.membersParty[2].votes / this.membersParty[2].total
            );
          } else {
            this.membersParty[2].votesPct = 0;
          }
